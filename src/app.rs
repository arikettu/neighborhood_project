...
pub(crate) fn update() {
    ...
    for i in timers.iter_mut() {
        let real_diff = i.2.duration_since(Instant::now());
        if real_diff.as_secs() != i.1.as_secs() {
            needs_update = true;
            i.1 = real_diff;
        }
    }
    if needs_update { crate::APP_HANDLE.get().unwrap().upgrade_in_event_loop(|x| x.set_timers(slint::ModelRc::from(TIMERS.read().unwrap().iter().map(|times| times.1.as_secs() as i32).collect::<Vec<i32>>().as_slice()))).unwrap() };
}